...
export const RunResultSchema = z.object({
  ...
  reasoning_tokens: z.number().int().min(0).optional().default(0),
  total_tokens: z.number().int().min(0),
  requested_model: z.string().min(1).optional(),
  response_model: z.string().min(1).optional(),
  provider_response_ids: z.array(z.string().min(1)).optional().default([]),
...
export const AggregateSchema = z.object({
  ...
  ineligible_run_reasons: z.record(z.string()).optional().default({}),
  ...
  failure_counts: z.record(z.number().int().min(0)).optional().default({}),
  cpr_usd: DecimalUsdSchema.nullable(),
  cpr_defined: z.boolean().optional().default(true),
  cpr_undefined_reason: z.string().min(1).nullable().optional(),
  ...
  formula_version: z.string().min(1).optional().default("cpr_failure_inclusive_v1"),
  quality_floor_pass_rate: z.number().min(0).max(1).optional().default(0.75),
  minimum_attempts: z.number().int().min(1).optional().default(2),
  source_result_digest: Sha256HashSchema.optional().default("0".repeat(64)),
  created_at: TimestampSchema,
}).strict();
...
export const DecisionReceiptSchema = z.object({
  ...
  trigger_event_id: EventIdSchema.nullable().optional(),
  fingerprint_id: FingerprintIdSchema.nullable().optional(),
  plan_id: PlanIdSchema.nullable().optional(),
  baseline_policy_version: PolicyVersionSchema.nullable().optional(),
  candidate_policy_version: PolicyVersionSchema.nullable().optional(),
  selected_task_ids: z.array(z.string()).optional().default([]),
  eligible_run_keys: z.array(LogicalRunKeySchema).optional().default([]),
  excluded_run_reasons: z.record(z.string()).optional().default({}),
  baseline_evidence: z.record(z.unknown()).optional().default({}),
  candidate_evidence: z.record(z.unknown()).nullable().optional(),
  approval_boundary_version: z.string().min(1).optional().default("decision_policy_v1"),
  rollback_performed: z.boolean().optional().default(false),
  publication_status: z.literal("PUBLISHED").optional().default("PUBLISHED"),
